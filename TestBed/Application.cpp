...
int main() {
	...
	Graphics::Mesh* mesh2 = Graphics::Mesh::MakeBox(Math::Vector3(0), 1);

	Physics::BoundingBox* bounds = new Physics::BoundingBox(Math::Vector3(0), 1);
	Physics::BoundingBox* bounds2 = new Physics::BoundingBox(Math::Vector3(0), 1);
	...
	gameObject.AddComponent<Transform>(new Transform(gameObject, Math::Vector3(1.5f, 0, -5)));
	gameObject.AddComponent<RigidBody>(new RigidBody(gameObject, 1, 1, 1, false, Math::Vector3(0, 0, 0), Math::Vector3(0, 0, 1)));
	gameObject.AddComponent<Collider>(collider);

	gameObject2.AddComponent<RenderMesh>(renderMesh2);
	gameObject2.AddComponent<Transform>(new Transform(gameObject2, Math::Vector3(0, 0, -5)));
	gameObject2.AddComponent<RigidBody>(new RigidBody(gameObject2, 1, 1, 0, false, Math::Vector3(1, 0, 0), Math::Vector3(0, 0, 0)));
